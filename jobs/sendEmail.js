...
async function main() {
  ...
  const emailsCM = []; //OBTENER LA DIVISIÓN DE LOS MATERIALES CON ESTADO PENDIENTE
  ...
  const despachosPendientesObj = Object.values(JSON.parse(JSON.stringify(despachosPendientes))); // JUNTAS LA DIVISIONES EN UNA VARIABLE

  for (const m of despachosPendientesObj) {
    materialsDivisionId.push(Object.values(m));
  }

  const materialsData = materialsDivisionId.flat(); //OBTENER EMAIL Y DIVISIONES DE USUARIOS A LOS CUALES SE LE DEBE MANDAR LOS EMAIL
  ...
  const usersToSendEmailObj = Object.values(JSON.parse(JSON.stringify(usersToSendEmail)));
  ...
  const getFirstMaterialToDispatch = await pool.query("select date_in, materials_divisions from materials where materials_divisions in (?) and status = 1 group by materials_divisions", [materialsData]);
  const getFirstMaterialToDispatchObj = Object.values(JSON.parse(JSON.stringify(getFirstMaterialToDispatch)));
