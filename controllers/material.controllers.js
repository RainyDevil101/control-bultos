...
const pool = require("../database/database"); // Get users
...
const getMaterial = async (req = request, res = response) => {
  ...
  const getUser = req.user;
  const userDivisionsId = getUser[0].users_divisions;
  const material = await pool.query("SELECT * FROM materials WHERE id = ? AND materials_divisions = ?", [id, userDivisionsId]);
  res.status(200).json({
    material
  });
};
...
const createMaterial = async (req = request, res = response) => {
  ...
  const alarm_one = new Date(alarm); // const alarm_two = new Date();
  ...
  try {
    ...
    const usersEmail = await pool.query("SELECT email FROM users WHERE users_divisions = ? AND email != 'PENDIENTE'", [userDivision]);
    const usersEmailJson = Object.values(JSON.parse(JSON.stringify(usersEmail)));
