...
const bcryptjs = require("bcryptjs"); // Get users


const getUsers = async (req = request, res = response) => {
  const users = await pool.query("SELECT id, fullname, fulllastname, email, rut, role, position, users_divisions FROM users WHERE status = 1");
  res.status(200).json({
    users
  });
};
...
const deleteUser = async (req = request, res = response) => {
  ...
  res.status(200).json({
    msg: "El usuario fue eliminado correctamente" // uid,
    // userAuth
