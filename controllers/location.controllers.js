...
const getLocations = async (req = request, res = response) => {
  const getLocations = await pool.query("SELECT * FROM locations WHERE status = 1 ORDER BY name ASC");
...
const updateLocation = async (req = request, res = response) => {
  ...
  await pool.query("UPDATE locations SET ? WHERE id = ? AND status = 1", [ubication, id]);
  res.status(200).json({
